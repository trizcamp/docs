...
description: Endpoint to create Fallback
api: 'POST https://chat.api.toolzz.com.br/api/v1/bot/configuration/fallback'
---
<Warning>User must contain a credential that is not being used by the bot</Warning>

<Expandable title="Payload">
<ResponseField name="botId" type="uuid" required>
  Unique identifier of the bot.
</ResponseField>
<ResponseField name="fallBacks" type="array" required>
  List of fallback configurations for the bot.
</ResponseField>
<ResponseField name="fallBacks[].order" type="integer" required>
  The order of the fallback configuration in the list.
</ResponseField>
<ResponseField name="fallBacks[].credentialId" type="uuid" required>
  Unique identifier of the credential used for the fallback.
</ResponseField>
<ResponseField name="fallBacks[].model" type="string" required>
  The model name used for the fallback.
</ResponseField>
</Expandable>
